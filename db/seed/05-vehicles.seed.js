...
const catalog = rentalCars.map((car) => ({ ...car, brand: car.name === "All New Rush" ? "Toyota" : car.name.startsWith("Range Rover") ? "Range Rover" : car.name.split(" ")[0] }));
...
export async function seedVehicles(brandRows, categoryRows, subCategoryRows) {
  ...
  const subCategoryIds = new Map(subCategoryRows.map((subCategory) => [subCategory.name, subCategory.id]));
  return db.insert(vehicles).values(catalog.map((car) => ({
    name: car.name,
